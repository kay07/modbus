...
public class WriteRegistersRequest extends ModbusRequest {
     ...
     */
    public WriteRegistersRequest(int slaveId, int startOffset, short[] sdata,boolean b) throws ModbusTransportException {
        super(slaveId);
    ...
    protected byte[] cToBytes(short[] sdata) {
        int byteCount = sdata.length * 2;
        byte[] data = new byte[byteCount];

        for(int i = 0; i < sdata.length; ++i) {
            data[i * 2+1] = (byte)(255 & sdata[i] >> 8);
            data[i * 2 ] = (byte)(255 & sdata[i]);
        }
